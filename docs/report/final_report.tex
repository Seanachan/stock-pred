...
整體管線由三個彼此獨立、僅共用交易 API 的子系統構成（圖~\ref{fig:pipeline}）：

\begin{enumerate}[leftmargin=2em]
  \item \textbf{資料層}（\texttt{stock\_api/}）：直接抓取 TWSE／TPEX 官方端點之價量歷史，統一為 10 欄位 schema，內建指數退避、ROC 日期轉換與千分位／除權息標記清理；每月請求間隔 2 秒以避免上游限流。
  \item \textbf{模型／部署層}（\texttt{RL/}）：特徵工程、LSTM 評分網路、集成決策與實盤下單。
  \item \textbf{CI 自動化}：兩條 GitHub Actions——每日部署（工作日 17:00 台北）與每日重訓——將狀態與權重提交回 \texttt{main}。
\end{enumerate}
...
\subsection{特徵工程}
每檔股票每日構造 14 維特徵：8 維\emph{個股本地技術指標}（\texttt{return}、\texttt{bias\_5}、\texttt{bias\_20}、\texttt{macd\_h}、\texttt{rsi\_14}、\texttt{bb\_pos}、\texttt{atr}、\texttt{capacity\_change}）、4 維\emph{橫斷面每日排名}（report、rsi、bias\_20、量能變化之 pct-rank），以及 v6 新增之 \texttt{vol\_20}（20 日對數報酬標準差）與 \texttt{mom\_60}（60 日動能）。橫斷面排名提供「市場相對」訊號，是模型超額報酬的關鍵來源。
...
網路對每檔股票之 50 日特徵窗（含 4 維可學習個股嵌入 \texttt{nn.Embedding}）跑\emph{共享權重}的單層 LSTM，取末步隱狀態經線性層輸出一個分數；$N$ 檔分數與一個可學習現金分數串接後 softmax，得到含現金槽的權重向量。其後施以每檔上限 \texttt{max\_weight}$=0.10$。模型主要參數量與 $N$ 幾乎無關（LSTM 共享），僅嵌入表 $N\times 4$ 隨宇宙線性微增，故由 46 擴至 88 在計算上乾淨可行。

\subsection{損失函數}
端到端最小化\textbf{負 Sharpe}。實現報酬序列時計入 L1 換手成本：
\[
\text{pnl}_t \;=\; \langle w_t,\, r_t\rangle \;-\; c\cdot \big\| w_t - \tilde{w}_{t-1}\big\|_1,
\]
其中 $\tilde{w}_{t-1}$ 為前期權重經當期資產漂移後之值，$c=0.0042$。如此模型在訓練時即「感知」交易成本，傾向降低不必要換手。

\subsection{三代演進與兩個關鍵槓桿}
\begin{description}[leftmargin=1.5em, style=nextline]
  \item[v4（基線）] 5 seed 以\emph{逐維中位數}集成。問題：中位數會抹除任一 seed 為零的高信念部位——單 seed walk-forward 超額 +4.09\%，中位集成卻塌陷為 +0.98\%。
  \item[v5—槓桿 A：注水式溢出（water-fill）] 原本「超過上限的權重一律倒入現金」造成在多頭時結構性落後等權基準（約 48\% 資金閒置於現金）。改為將溢出\emph{按現價權重重分配給未達上限之個股}（保留模型深思熟慮的現金，僅消除機械式溢出），現金佔比由 $\sim$48\% 降至 $\sim$3\%。
  \item[v5—槓桿 B：Sharpe 加權集成 + 集成 Top-$N$ 截斷] 以 $\mathrm{softmax}(\text{val Sharpe})$ 對 seed 加權平均取代中位數；再於\emph{集成後}硬性保留權重最大的 $N$ 檔、其餘歸零後重新標準化。三處上限邏輯（訓練前傳、walk-forward 集成、部署集成）以單一共享函數鎖定一致，並以對拍測試保證 backtest = live。
  \item[v6—宇宙擴充] 88 檔 + Top-$N$ 重調 + 每日重訓。
\end{description}
...
\subsection{現金外溢槓桿之實證}
v5 的注水改造將 fold-0（強多頭）之現金由 54.7\% 降至 10.5\%、整體超額由 v4 等級提升至 +4.27\%（平均，10 折），驗證「現金外溢」確為當時的主要拖累。但其平均值高度受單一 fold-4（+53.7\%）驅動，故全程以\emph{中位數}為穩健衡量。
