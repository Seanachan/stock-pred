...
Repo bundle **two independent Python subprojects** sharing simulated-trading API (NCKU `ciot.imis.ncku.edu.tw/sim_stock`) but otherwise no dependency:
...
No test suite, no build step, no CI. `pylint` referenced in `.vscode/settings.json` only to *disable* missing-docstring warnings.
...
- Code, comments, docstrings, example data predominantly **Traditional Chinese** — preserve language when edit.
- Two subprojects intentionally duplicate trading-API client (`Buy_Stock`/`Sell_Stock`/`Get_User_Stocks`); evolved separately, so do not unify without explicit ask.